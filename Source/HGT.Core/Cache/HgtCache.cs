...
namespace HGT.Core
{
    public static class HgtCache
    {
        ...
        public static void Insert(string key, object obj)
        {
            ...
            HgtContext ctx = HgtContext.Current;
            if (ctx != null && ctx.IsWebRequest)
                durations = 123;// get timeout form config
        ...
        private static string GetHgtKey(string key)
        {
            if (!key.StartsWith("Hgt-Cache:"))
                key = string.Format("Hgt-Cache:{0}", key);
